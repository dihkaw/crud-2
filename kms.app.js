...
async function initializeDatabase() {
  ...
  if (rows[0].count === 0) {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      const filename = `products/${Date.now()}-${dummyImages[i]}`;
      await s3.send(new PutObjectCommand({
        Bucket: process.env.S3_BUCKET_NAME,
        Key: filename,
        Body: fileStream,
        ServerSideEncryption: 'aws:kms' // Memastikan enkripsi KMS dengan SigV4
      }));
...
app.post('/create', upload.single('image'), async (req, res) => {
  ...
  await s3.send(new PutObjectCommand({
    ...
    Body: fileStream,
    ServerSideEncryption: 'aws:kms' // Memastikan SigV4 diperlukan
  }));
